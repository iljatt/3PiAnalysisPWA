...
def find_files(SIGNATURE):
    ...
    if(VERBOSE == True):
        print("size of file list is %d" % str(len(file_list)))
...
def main(argv):
    ...
    HADD_RUNS_TOT = "hadd -fk %s_sum_%d_%d.root %s_sum_*_%d_%d.root" % (BASENAME, MINRUN, MAXRUN, BASENAME, MINRUN, MAXRUN)    
    HADD_RUNS_AMO = "hadd -fk %s_sum_AMO_%d_%d.root" % (BASENAME, MINRUN, MAXRUN)
    HADD_RUNS_PARA = "hadd -fk %s_sum_PARA_0_%d_%d.root" % (BASENAME, MINRUN, MAXRUN)
    HADD_RUNS_PERP = "hadd -fk %s_sum_PERP_90_%d_%d.root" % (BASENAME, MINRUN, MAXRUN)
    HADD_RUNS_PARA_45_135 = "hadd -fk %s_sum_PARA_135_%d_%d.root" % (BASENAME, MINRUN, MAXRUN)
    HADD_RUNS_PERP_45_135 = "hadd -fk %s_sum_PERP_45_%d_%d.root" % (BASENAME, MINRUN, MAXRUN)
    ...
    for RCDB_RUN in RCDB_RUNS:
        ...
        HIST_FILE = tuple(filter(lambda a: FORMATTED_RUN in a, FOUND_HIST_FILES))[0]
