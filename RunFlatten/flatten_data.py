#!/usr/bin/env python

##########################################################################################################################

from optparse import OptionParser
import os.path
import os
import sys
import re
import subprocess
import glob

#################################################### GLOBAL VARIABLES ####################################################

VERBOSE = False

####################################################### FIND FILES #######################################################

def find_files(SIGNATURE):

    # SEARCH FOR THE FILES
    file_signature = SIGNATURE
    file_list = glob.glob(file_signature)
    if(VERBOSE == True):
        print ("size of file_list is " + str(len(file_list)))

    return file_list

def main(argv):
    parser_usage = "flatten_data.py directory filePrefix"
    parser = OptionParser(usage = parser_usage)
    (options, args) = parser.parse_args(argv)

    if(len(args) != 2):
        parser.print_help()
        return

    # GET ARGUMENTS
    DIR = args[0]
    PREFIX = args[1]
    
    RUN_PERIOD = "2017-01"
    SUFFIX = ".root"

    # GET LIST OF FILES ON DISK
    os.chdir(DIR)
    FOUND_TREE_FILES = find_files( PREFIX + "*.root" )


    # ADJUST ONLY THE OUTPUT PATH
    outputPath = "/volatile/halld/home/ilbelov/Swif/flat_data"  
    totalScriptName = "%s/scripts/dataSubmitJobs_flatten_trees_%s.sh" % (outputPath,RUN_PERIOD)
    outBaseDir = "%s/dataTrees/RunPeriod-%s" % (outputPath,RUN_PERIOD)
    outScriptsDir = "%s/scripts" % outputPath
    outSubScriptsDir = "%s/scripts/subScripts" % outputPath
    

    
    if not os.path.exists(outBaseDir):
        os.makedirs(outBaseDir)
 
    if not os.path.exists(outScriptsDir):
        os.makedirs(outScriptsDir)    

    if not os.path.exists(outSubScriptsDir):
        os.makedirs(outSubScriptsDir)    
        

        
    # FIND/ADD JOBS
    for TREE_FILE in FOUND_TREE_FILES:
        if PREFIX not in TREE_FILE:
            continue
        if SUFFIX not in TREE_FILE:
            continue
#        print( TREE_FILE )

        # EXTRACT RUN NUMBER FOR INDEXING
        TEMP_FILE_INDEX = TREE_FILE.replace( PREFIX, '' )
#        print( TEMP_FILE_INDEX )
        FILE_INDEX = TEMP_FILE_INDEX.replace( SUFFIX, '' )
#        print( FILE_INDEX )

        # Write script to submit all jobs
        f = open(totalScriptName,'a')
        f.write("sbatch %s/scripts/subScripts/job_trees_flatten_data_%s.sh\n" % (outputPath,FILE_INDEX) )
        f.close() 

        # Write individual scripts for each job
        f2 = open( "%s/scripts/subScripts/job_trees_flatten_data_%s.sh" % (outputPath,FILE_INDEX), "w")
        f2.write( "#!/bin/sh\n" )
        f2.write( "#SBATCH -q phi\n" )
        f2.write( "#SBATCH -t 00:50:00\n" )
        f2.write( "/work/halld2/home/aschertz/hd_utilities/FlattenForFSRoot/flatten -in %s/%s -out %s/tree_pippippimpim__B4_flat_%s.root -mc 0 -chi2 10  -usePolarization 1 -combos 2\n" % (DIR,TREE_FILE,outBaseDir,FILE_INDEX) )
        f2.close()      


if __name__ == "__main__":
    main(sys.argv[1:])
    
