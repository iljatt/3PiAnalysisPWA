...
void skimTreeDeltaCut_ULsep(const char *infilename, const char *outfilename, const char *mode)
{

  
  const char *treename = "kin"; //remains intact

  //precuts and delta cuts
  //   const char *deltacuts_both = "Chi2Ndof <= 5. && E_Beam >= 8.2 && E_Beam <= 8.8 && Piplus2Proton_M >= 1.15 && Piplus2Proton_M <= 1.35 && Piplus1Proton_M >= 1.15 && Piplus1Proton_M <= 1.35";
  //   const char *deltacuts_1st = "Chi2Ndof <= 5. && E_Beam >= 8.2 && E_Beam <= 8.8 && Piplus1Proton_M >= 1.15 && Piplus1Proton_M <= 1.35 && Piplus2Proton_M > 1.35 && tvar2 >= -2.";
  //   const char *deltacuts_2nd = "Chi2Ndof <= 5. && E_Beam >= 8.2 && E_Beam <= 8.8 && Piplus2Proton_M >= 1.15 && Piplus2Proton_M <= 1.35 && Piplus1Proton_M > 1.35 && tvar1 >= -2.";


  const char *deltacuts_both = "Piplus2Proton_M >= 1.15 && Piplus2Proton_M <= 1.35 && Piplus1Proton_M >= 1.15 && Piplus1Proton_M <= 1.35";
  const char *deltacuts_1st  = "Piplus1Proton_M >= 1.15 && Piplus1Proton_M <= 1.35 && Piplus2Proton_M > 1.35 && tvar2 >= -2.";
  const char *deltacuts_2nd  = "Piplus2Proton_M >= 1.15 && Piplus2Proton_M <= 1.35 && Piplus1Proton_M > 1.35 && tvar1 >= -2.";

  
  
  TCut cut;
  if (strncmp(mode,"1st",3)==0) cut = deltacuts_1st;
  else if (strncmp(mode,"2nd",3)==0) cut = deltacuts_2nd;
  else if (strncmp(mode,"both",4)==0) cut = deltacuts_both;
  else return;
  ...
  TTree *copiedtree_a = tree->CopyTree(cut);

  tree->SetBranchStatus("*",0);
  tree->SetBranchStatus("Chi2Ndof",1);
  tree->SetBranchStatus("E_Beam",1);
  tree->SetBranchStatus("Piplus*Proton_M",1);
  tree->SetBranchStatus("tvar*",1);
  tree->SetBranchStatus("Piplus*P4",1);
  TTree *copiedtree_b = tree->CopyTree(cut);
  
  //********************Rename simple flat branches in the copiedtree_a and clone it*******************
  replaceNamesOfBranches(copiedtree_a,mode);     
  copiedtree_a->SetBranchStatus("trash*",0);  
  ...
  copy4vectors(bufftree,newtree,mode);   
