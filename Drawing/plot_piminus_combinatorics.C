...
void plot_piminus_combinatorics()
{
  ...
  gROOT->SetBatch(kTRUE);
    ...
    const char *filename = "histos_data_spring2017_precuts_diffpipluscut_deltacut_fourpionscut.root";
  //  const char *filename = "Different_piminuscuts/histos_data_spring2017_precuts_diffpipluscut_deltacut_fourpionscut_piminuscut0p4.root";
   ...
   if (!file) return;
    ...
    TH1D *fourpions_mass = (TH1D*)file->TFile::Get("FourPions_M");
    fourpions_mass->GetXaxis()->SetRangeUser(1.5,3.5);
    ...
    TH1D *deltatheta_wrong = (TH1D*)file->TFile::Get("DeltaTheta_LV_UV_wrong");
      ...
      defstyle(fourpions_mass); fourpions_mass->SetMarkerColor(kBlack);
      ...
      protonpiplusLpiminus2_mass->GetXaxis()->SetTitle("m(p#pi^{+}#pi^{-})  [GeV]");
      ...
      fourpions_mass->GetXaxis()->SetTitle("m(4#pi)  [GeV]");
      ...
      //      protonpiplusUpiminus1_mass->SetMaximum(35000.);//35000.
      //      piminus2_p->SetMaximum(35000.);
      

      TLegend *leg = new TLegend(0.1,0.3,0.8,0.7); 
      leg->AddEntry(piminus1_p, "Combinations with #pi^{-}_{fast}  ", "p");
      leg->AddEntry(piminus2_p, "Combinations with #pi^{-}_{slow}  ", "p");

      
      TLegend* leg5 = new TLegend(0.1,0.68,0.3,0.89);   //(0.4,0.68,0.6,0.89); -- in the middle
      leg5->AddEntry(protonpiplusLpiminus1_mass, "#pi^{+}(lv) #pi^{-}(fast)  ", "p");
      leg5->AddEntry(protonpiplusUpiminus1_mass, "#pi^{+}(uv) #pi^{-}(fast)  ", "p");
      leg5->AddEntry(protonpiplusLpiminus2_mass, "#pi^{+}(lv) #pi^{-}(slow)  ", "p");
      leg5->AddEntry(protonpiplusUpiminus2_mass, "#pi^{+}(uv) #pi^{-}(slow)  ", "p");
	...
	piplusUpiminus2_mass->Draw("P SAME");
	canva->cd(2);
	...
	canva->cd(4);
	piminus2proton_mass->Draw("P");
        piminus1proton_mass->Draw("P SAME");
	canva->cd(5);
        protonpiplusLpiminus2_mass->Draw("P");
	protonpiplusUpiminus2_mass->Draw("P SAME");
	protonpiplusLpiminus1_mass->Draw("P SAME");
	protonpiplusUpiminus1_mass->Draw("P SAME");
	//	leg5->Draw("SAME");
	canva->cd(6);
